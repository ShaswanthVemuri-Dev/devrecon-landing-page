...
const MiniMenuButton = ({ open, onClick }) => (
  <button
    ...
    aria-expanded={open}
    onClick={onClick}
    className={`mini-preview-menu-button relative z-50 flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-transparent text-white transition duration-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#4A90E2]/50 hover:bg-white/[0.06] ${open ? 'is-open bg-white/[0.06]' : ''}`}
  >
    <span className="mini-preview-menu-ripple absolute inset-0 rounded-full bg-[#4A90E2]" />
...
const MyMedicalsBrand = ({ website }) => (
  <a href={website} target="_blank" rel="noreferrer" className="flex min-w-0 items-center gap-2.5 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-[#4A90E2]/50">
    <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-[#1C1C1E] shadow-lg">
      <img src={publicAsset(PREVIEW_ASSETS.logo)} alt="MyMedicals logo" className="colour-logo h-full w-full object-contain" draggable={false} />
    </div>
...
const MyMedicalsPhone = () => (
  <div className="mymedicals-phone" aria-hidden="true">
    <div className="relative h-full overflow-hidden rounded-[1.1rem] bg-[#050505] min-[740px]:rounded-[1.26rem] min-[900px]:rounded-[1.36rem] min-[1280px]:rounded-[1.5rem]">
      <img src={publicAsset(PREVIEW_ASSETS.phoneHero)} alt="" className="native-colour-stage absolute inset-0 h-full w-full object-cover" draggable={false} />
    ...
    <ProductPreviewFrame product={product} open={open} variant="mymedicals">
      ...
      <div className="product-preview-layout mymedicals-preview-layout text-white">
        <div className="product-preview-copy mymedicals-preview-copy">
          <div className="native-colour-stage absolute -left-10 -top-10 h-32 w-32 rounded-full bg-[#4A90E2]/18 blur-3xl devrecon-float-soft" />
          ...
          <div className="relative z-10">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <a href={product.website} target="_blank" rel="noreferrer" className="native-colour-stage inline-flex items-center gap-2 rounded-full bg-[#4A90E2] px-5 py-3 text-[10px] font-bold uppercase tracking-[0.14em] text-white visited:text-white active:text-white focus:text-white hover:text-white shadow-[0_12px_32px_rgba(74,144,226,0.20)] transition duration-500 hover:scale-[1.02] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#4A90E2]/50">
                View site
